...
class Board extends Component{
	...
	render(){
		...
		for (var i = 0; i < this.props.flop.length; i++){
			flop.push(<div 	key={i} 
							className="col-xs-2"><Card number={this.props.flop[i].number}
						 	suit={this.props.flop[i].suit}/></div>)			

		}
			for (var i = 0; i < this.props.turn.length; i++){
			turn.push(<div 	key={i} 
							className="col-xs-2"><Card number={this.props.turn[i].number}
						 	suit={this.props.turn[i].suit}/></div>)			

		}
			for (var i = 0; i < this.props.river.length; i++){
			river.push(<div 	key={i} 
							className="col-xs-2"><Card number={this.props.river[i].number}
